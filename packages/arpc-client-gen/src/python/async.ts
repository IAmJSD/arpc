...
function initGen(client: Client) {
    ...
    const superInit = `
        ...
        super().__init__("version=${client.apiVersion}", hostname, init_headers, loop)`;

    // Handle the no auth case.
    if (!client.authentication) {
        return `    def __init__(self, hostname: str = None, loop: asyncio.AbstractEventLoop = None):
    ...
    return `    def __init__(self, token: str = None, ${tokenType}, hostname: str = None, loop: asyncio.AbstractEventLoop = None):
        init_headers = {}
        if token:
...
export function pythonAsync(data: BuildData) {
    const chunks = [header];
    ...
    for (const c of data.clients) {
        chunks.push(buildClient(true, data.enums, c, initGen));
    }
