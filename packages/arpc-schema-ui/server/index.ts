...
function generateHtmlForBuildData(buildData: BuildData, selectedVersion: string) {
    ...
    if (buildData.objects.length > 0) {
        chunks.push(titleRenderer("Objects", 2));
        chunks.push(/* html */`<p>These are the objects in this API.</p>`);
    ...
    if (buildData.enums.length > 0) {
        chunks.push(titleRenderer("Enums", 2));
        chunks.push(/* html */`<p>These are the enums in this API.</p>`);
        for (const enum_ of buildData.enums) {
            chunks.push(enumRenderer(enum_));
        }
    }

    chunks.push(titleRenderer("Exceptions", 2));
    chunks.push(/* html */`<p>The following exceptions are built into arpc:</p>
<ul class="list-disc">`);
    ...
    if (buildData.customExceptions.length > 0) {
        chunks.push(/* html */`<p>The following exceptions are defined by the API:</p>
<ul class="list-disc">`);
...
export function render(title: string, description: string, buildData: BuildData) {
    function generateDynamicHtml(selectedVersion: string) {
        ...
        return /* html */`<div id="arpc_schema" class="hidden">${JSON.stringify(buildData)}</div>
${generateHtmlForBuildData(buildData, selectedVersion)}`;
    }
